...
export default function App() {
  ...
  return <div>
    <div style={{position:"sticky",top:0,zIndex:150,background:"rgba(243,244,239,.96)",backdropFilter:"blur(10px)",borderBottom:"1px solid rgba(50,56,42,.12)",padding:"9px 18px"}}><div style={{maxWidth:1560,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,flexWrap:"wrap"}}><div style={{fontSize:12,color:"#647062"}}><strong style={{color:"#20251F"}}>Costa Gear Sourcing</strong> · Sourcing, buying, logistics and inventory control</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
      <button style={navButton(workspace==="dashboard")} onClick={()=>setWorkspace("dashboard")}>Dashboard</button>
    ...
    {workspace==="dashboard"?<OperationalDashboard onNavigate={setWorkspace}/>:workspace==="operations"?<><LegacyApp/><SourcingIntelligencePanel/></>:workspace==="shipments"?<ShipmentAllocationWorkspace/>:workspace==="importcosts"?<ImportCostWorkspace/>:workspace==="buying"?<BuyingDecisionWorkspace/>:workspace==="receiving"?<ReceivingInventoryWorkspace/>:<SourcingDecisionLab/>}
  </div>;
}
