...
export default function SupplierQuotationWorkspace({onNavigate,initialQuotationId=null,onOpenIntake}){
  const[loading,setLoading]=useState(true),[error,setError]=useState(""),[message,setMessage]=useState("");
  const[suppliers,setSuppliers]=useState([]),[products,setProducts]=useState([]),[productTypes,setProductTypes]=useState([]),[categories,setCategories]=useState([]),[materials,setMaterials]=useState([]),[vehicleFitments,setVehicleFitments]=useState([]),[quotations,setQuotations]=useState([]),[orders,setOrders]=useState([]),[lines,setLines]=useState([]);
  ...
  const addTypeFromQuotation=async()=>{
    const name=window.prompt("New Product Type name:");
    if(!name?.trim())return;
    const familyCode=window.prompt("SKU family code (2-4 letters/numbers), e.g. RR:");
    if(!familyCode?.trim())return;
    setBusy(true);setError("");
    try{
      const row=await addProductType({name,familyCode});
      if(row){
        setProductTypes(prev=>[...prev.filter(x=>x.id!==row.id),row].sort((a,b)=>a.name.localeCompare(b.name)));
        setNewProductForm(v=>({...v,productType:row.name}));
      }
    }catch(e){setError(e.message||"Unable to add Product Type.");}
    finally{setBusy(false);}
  };
  const addCategoryFromQuotation=async()=>{
    const name=window.prompt("New Product Category name:");
    if(!name?.trim())return;
    setBusy(true);setError("");
    try{
      const row=await addProductCategory(name);
      if(row){
        setCategories(prev=>[...prev.filter(x=>x.id!==row.id),row].sort((a,b)=>(a.sort_order-b.sort_order)||a.name.localeCompare(b.name)));
        setNewProductForm(v=>({...v,category:row.name}));
      }
    }catch(e){setError(e.message||"Unable to add Product Category.");}
    finally{setBusy(false);}
  };
  ...
  return <div className="cg-supplier-quotation-workspace" style={{minHeight:"100vh",background:C.soft,color:C.ink}}>
    ...
    <div className="cg-supplier-quotation-body" style={{padding:"16px 0 28px",display:"grid",gap:14}}>
      ...
      <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:13,padding:"10px 12px",display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}>
        <div><strong style={{fontSize:12.5}}>Single intake channel</strong><div style={{fontSize:10.5,color:C.muted,marginTop:2}}>New supplier PDFs, Excel files, catalogs and quotations enter through Supplier Intake. This workspace starts after intake is complete.</div></div>
        <button type="button" onClick={onOpenIntake} style={btn(false)}>Open Supplier Intake</button>
      </div>
