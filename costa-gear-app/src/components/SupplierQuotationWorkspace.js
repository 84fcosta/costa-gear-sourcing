...
export default function SupplierQuotationWorkspace({onNavigate}){
  ...
  const doImport=async()=>{if(!preview||!importSupplierId)return setError("Select the supplier in Costa Gear before importing.");setBusy(true);setError("");try{const sourceRef=preview.header.quoteRef;const created=await importSupplierQuotation({supplierId:importSupplierId,header:preview.header,lines:preview.lines});setPreview(null);setImportSupplierId("");await load();setSelectedId(created.id);setMessage(sourceRef?`Quotation ${created.quote_ref} imported. Supplier reference preserved.`:`Quotation ${created.quote_ref} imported. Costa Gear reference generated because the supplier did not provide one.`);}catch(e){setError(e.message||"Unable to import quotation.");}finally{setBusy(false);}};
      ...
      {loading?<div style={{padding:30,textAlign:"center",color:C.muted}}>Loading quotations…</div>:<div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:14,alignItems:"start"}}>
        ...
        <div style={{display:"grid",gap:14}}>{selected?<>
          ...
          <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:13,padding:14,overflowX:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",marginBottom:9}}><div><strong>Product Matching</strong><div style={{fontSize:10.5,color:C.muted}}>Resolve each supplier item by matching an existing CG product, creating a new product, or ignoring the item. Ignored lines remain in the original quotation but are excluded from Product Master, Decision Lab and Buying Drafts.</div></div><div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:10.5,color:C.muted}}>{matched} matched · {ignored} ignored</span>{allResolved&&badge("RESOLVED")}</div></div>
            <table style={{width:"100%",borderCollapse:"collapse",minWidth:1360,fontSize:11.5}}><thead><tr style={{background:"#F5F7F1"}}>{["Line","Supplier SKU","Supplier Description","Qty","Unit Price","Line Total","Validation","Costa Gear Product / Comparison","Match"].map(h=><th key={h} style={{textAlign:"left",padding:8,color:C.muted,borderBottom:"1px solid "+C.border}}>{h}</th>)}</tr></thead><tbody>{lines.map(l=>{const p=productById(l.product_id);const isIgnored=l.match_status==="IGNORED";return <tr key={l.id} style={{background:isIgnored?"#F7F8F5":"#fff"}}><td style={{padding:8,borderBottom:"1px solid "+C.border}}>{l.line_no}</td><td style={{padding:8,borderBottom:"1px solid "+C.border,fontFamily:"monospace",fontWeight:800}}>{l.supplier_sku||"—"}</td><td style={{padding:8,borderBottom:"1px solid "+C.border,minWidth:240}}>{l.supplier_description||"—"}</td><td style={{padding:8,borderBottom:"1px solid "+C.border}}>{l.quantity} {l.unit||""}</td><td style={{padding:8,borderBottom:"1px solid "+C.border}}>{money(l.unit_price,selected.currency)}</td><td style={{padding:8,borderBottom:"1px solid "+C.border}}>{money(l.supplier_line_total??l.calculated_line_total,selected.currency)}</td><td style={{padding:8,borderBottom:"1px solid "+C.border}}>{badge(l.line_validation)}</td><td style={{padding:6,borderBottom:"1px solid "+C.border,minWidth:530}}>
  {isIgnored?<div style={{border:"1px solid "+C.border,borderRadius:9,padding:9,background:"#F3F4EF"}}><div style={{fontSize:11.5,fontWeight:850}}>Ignored for Costa Gear catalog</div><div style={{fontSize:10.5,color:C.muted,marginTop:2}}>This line remains part of the supplier quotation and mathematical validation.</div>{selected.status==="Imported"&&<button type="button" disabled={busy} style={{...btn(),marginTop:6,padding:"6px 9px"}} onClick={()=>setIgnored(l,false)}>Restore / Reconsider</button>}</div>:<>
    <select style={input} value={l.product_id||""} disabled={selected.status==="Converted"||busy} onChange={e=>mapLine(l.id,e.target.value)}><option value="">Select existing CG product</option>{products.map(x=><option key={x.id} value={x.id}>{productOptionLabel(x)}</option>)}</select>
    {p&&<ProductComparison line={l} product={p} currency={selected.currency}/>}
    {selected.status==="Imported"&&<div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>{!p&&<button type="button" style={{...btn(),padding:"6px 9px",color:C.oliveDark,borderColor:"rgba(133,140,56,.35)"}} onClick={()=>openCreateProduct(l)}>+ Create New Product</button>}<button type="button" disabled={busy} style={{...btn(),padding:"6px 9px",color:C.muted}} onClick={()=>setIgnored(l,true)}>Ignore Item</button></div>}
  </>}</td><td style={{padding:8,borderBottom:"1px solid "+C.border}}>{badge(isIgnored?"IGNORED":(l.product_id?"MATCHED":"UNMATCHED"))}</td></tr>})}</tbody></table>
          </div>
