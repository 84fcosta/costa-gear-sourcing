...
export default function LogisticsWorkspace({ initialView="shipments" }) {
  ...
  return <div>
    <div className="cg-subworkspace-header">
      <div className="cg-subworkspace-inner">
        <div>
          ...
          <div className="cg-subworkspace-copy">Manage shipments, freight, duty and import costs in a single workflow.</div>
        </div>
    ...
    <div className="cg-module-embedded">{view==="shipments" ? <ShipmentAllocationWorkspace/> : <ImportCostWorkspace/>}</div>
  </div>;
}
