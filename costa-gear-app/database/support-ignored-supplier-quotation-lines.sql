-- Allows Supplier Quotation lines to be resolved as MATCHED or IGNORED.
-- Applied to production via Supabase migration: support_ignored_supplier_quotation_lines
...
begin
  ...
  if exists(
    ...
    where quotation_id=p_quotation_id
      and (
        match_status not in ('MATCHED','IGNORED')
        or (match_status='MATCHED' and product_id is null)
      )
  ) then
    raise exception 'Resolve every quotation line by matching it to a Costa Gear product or ignoring it.';
  end if;
  ...
  if exists(
    ...
    from public.supplier_quotation_lines
    where quotation_id=p_quotation_id
      and line_validation='REVIEW REQUIRED'
  ) then
  ...
  set allocated_shipping_cad=null,
      allocated_shipping_per_unit_cad=null,
      quote_id=null
  where quotation_id=p_quotation_id
  ...
  loop
    ...
    v_alloc := case
      when v_shipping_known and v_q.shipping_total is not null and v_q.shipping_total>0
        then v_shipping_cad*(v_basis/v_total_basis)
    ...
    v_landed := case
      when v_shipping_known and v_duty_known
        then v_product_cad+coalesce(v_per_unit,0)+coalesce(v_duty,0)+coalesce(v_existing_brokerage,0)+coalesce(v_existing_other_fees,0)
      else null
    end;
...
CREATE OR REPLACE FUNCTION public.set_supplier_quotation_line_ignored(p_line_id uuid, p_ignored boolean)
 RETURNS supplier_quotation_lines
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
declare
...
begin
  ...
  if not found then
    raise exception 'Quotation line not found.';
  end if;

  select q.status
    into v_status
  from public.supplier_quotations q
  where q.id = v_line.quotation_id;

  if v_status <> 'Imported' then
    raise exception 'Items can only be ignored or restored before the quotation is finalized.';
  end if;
